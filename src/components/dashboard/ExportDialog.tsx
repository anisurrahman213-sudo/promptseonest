  ...
  const handleExport = useCallback(async () => {
    ...
    try {
      ...
      if (csvFiles.length > 1 && bundleAsZip) {
        ...
        const platformInstructions: Partial<Record<ExportFormat, string>> = {
          adobe_stock:
            '1. Log in to Adobe Stock Contributor portal: https://contributor.stock.adobe.com\n2. Upload your IMAGE files FIRST (drag & drop or via FTP).\n3. Wait until images finish processing.\n4. Go to "Upload" → click "Upload spreadsheet" and select each CSV file.\n5. Filenames in CSV must match uploaded image filenames EXACTLY.\n6. Max 5,000 rows per CSV (this export already complies).',
          shutterstock:
            '1. Log in to Shutterstock Contributor: https://submit.shutterstock.com\n2. Upload your IMAGE files first via the Upload page.\n3. Once uploaded, go to "Edit" → use "Bulk edit via CSV".\n4. Upload each CSV file from this ZIP one by one.\n5. Filenames must match exactly.',
          freepik:
            '1. Log in to Freepik Contributor: https://contributor.freepik.com\n2. Upload your image/vector files first.\n3. Use the bulk metadata import option.\n4. Upload each CSV from this ZIP.\n5. Make sure filenames match exactly.',
        };

        const instructions =
          platformInstructions[selectedFormat] ||
          `1. Log in to your ${platformName} contributor account.\n2. Upload your media files first.\n3. Use the bulk metadata / CSV import option.\n4. Upload each CSV file from this ZIP.\n5. Filenames in CSV must match uploaded files exactly.`;

        const readme = `${platformName} Metadata Export
${'='.repeat(platformName.length + 17)}

Generated: ${generatedAt}
Platform:  ${platformName}
Total items: ${downloadedCount}
...
Important Notes
---------------
• All CSV files are UTF-8 encoded with BOM (open in Excel as "CSV UTF-8").
• If you edit in Excel/Google Sheets, save as "CSV UTF-8" to keep encoding.
• Each CSV holds at most 5,000 rows (platform-safe limit).
• Upload your media files BEFORE the CSV.
• Filenames in CSV must match your uploaded file names exactly.

Generated by PromptSEONest — https://promptseonest.com
`;
